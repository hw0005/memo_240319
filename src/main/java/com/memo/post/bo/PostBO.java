...
public class PostBO {
	...
	public List<Post> getPostListByUserId(int userId, Integer prevId, Integer nextId) {
		...
		// 만약 4 3 2 페이지에 있을 떄
		// 1) 다음: 2보다 작은 3개 DESC
		// 2) 이전: 4보다 큰 개 ASC => 5 6 7 => BO에서 reverse 7 6 5
		// 3) 페이징 X: 최신순 3개 DESC
		// 3) -> 1) -> 2) 순
	...
	// input: 파라미터들
	// output: X
	public void updatePostByPostId(int userId, String loginId, int postId, String subject, String content, MultipartFile file) {
		
		// 기존글을 가져온다. (1. 이미지 교체시 삭제하기 위해 2. 업데이트 대상이 있는지 확인)
		Post post = postMapper.selectPostByPostIdUserId(userId, postId); // post 단건을 가져옴.
		...
		if (file != null) {
			...
			// 업로드 성공 시(imagePath가 null 아님) 기존 이미지가 있으면 제거
